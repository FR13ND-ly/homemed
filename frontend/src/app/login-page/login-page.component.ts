...
import { InvitationsService } from '../shared/data-access/invitations.service';
...
export class LoginPageComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.route.params
      .pipe(filter((params: any) => params.code))
      .subscribe((params) => {
  ...
  onGoogleLogin() {
    this.userService
      ...
      .subscribe((res: any): any => {
        if (res.exists)
          return this.router.navigate(['/dashboard/', res.type, 'main']);
  ...
  onRegister() {
    if (this.user.type == 'patient') {
      this.invitationsService.checkInvitation(this.user.invitationCode).subscribe((res: any) => {
        if (res.success) this.resolveRegister();
        else {
          this._snackBar.open('Codul de invitație nu este valid!', 'Închide', {duration: 5000});
        }
      })
    }
    else this.resolveRegister()
  }
